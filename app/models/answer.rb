...
class Answer < ApplicationRecord
  ...
  CLIENT          = Aws::LexModelBuildingService::Client.new(region: ENV['AWS_REGION'],
  ...
  def self.get_answer(answer, resp)
    if resp.intent_name == 'XANSd' && resp.slots["empFirstName"].present? && resp.slots["empLastName"].present?
      emp_id = User.find_by(first_name: resp.slots["empFirstName"], last_name: resp.slots["empLastName"]).employee_id
      "Your Employee ID is: #{emp_id}"
    else
      resp.message || answer.response
    end
  end
